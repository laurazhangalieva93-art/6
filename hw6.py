...
def get_correct_email(email_list: list[str]) -> list[str]:
    ...
    for email in email_list:
        ...
        if not domain_part:
            continue
        if any(email_clean.endswith(domain) for domain in valid_domains):
            correct_emails.append(email_clean)
    return correct_emails


def create_email(sender: str, recipient: str, subject: str, message: str) -> dict:
    """
    Создает словарь email с базовыми полями:
    'sender', 'recipient', 'subject', 'body'
    """
    email = {"sender": sender, "recipient": recipient, "subject": subject, "message": message}
    return email
...
def sender_email(recipient_list: list[str], subject: str, message: str, *, sender="default@study.com") -> list[
    ...
    normalized_message = clean_body_text(message)  # Нормализуем message
